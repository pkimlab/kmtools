...
def setup_module():
    ...
    if not op.isfile(BLAST_DB_ARCHIVE):
        ...
        urllib.request.urlretrieve(
            url="ftp://ftp.ncbi.nlm.nih.gov/blast/db/pdbaa.tar.gz", filename=BLAST_DB_ARCHIVE
        )
...
def test_1():
    ...
    #
    df = pd.read_csv(op.join(op.splitext(__file__)[0], "elaspic_training_set_core.tsv"), sep="\t")
...
def _blast_sequences(df, column):
    ...
    for i, row in df.iterrows():
        ...
        if counter > 100:
            break
    logger.info(
        "Performed {} blast '{}' searches in {:.1f} seconds...".format(
            N_BLASTS, column, time.time() - time_0
        )
    )
