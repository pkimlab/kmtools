...
@parametrize(
    "structure_file, r_cutoff, interchain, interaction_file_", TEST_DATA["test_get_interactions"]
)
...
def test_process_interactions_interface(
    ...
    processed_interaction_file_interface_ = op.join(
        TEST_FILES_DIR, processed_interaction_file_interface_
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            d["interaction_file_core"],
            d["interaction_file_core_aggbychain"],
            d["kept_idxs_"],
            d["kept_idxs_aggbychain_"],
        )
        for d in TEST_DATA["test_drop_duplicates_core"]
    ],
)
def test_drop_duplicates_core(
    interaction_file_core, interaction_file_core_aggbychain, kept_idxs_, kept_idxs_aggbychain_
):
    interaction_file_core = op.join(TEST_FILES_DIR, interaction_file_core)
    ...
    (
        interaction_core_unique,
        interaction_core_aggbychain_unique,
    ) = structure_tools.drop_duplicates_core(interaction_core, interaction_core_aggbychain)
...
@pytest.mark.parametrize(
    ...
    [
        (
            d["interaction_file_interface"],
            d["interaction_file_interface_aggbychain"],
            d["kept_idxs_"],
            d["kept_idxs_aggbychain_"],
        )
        for d in TEST_DATA["test_drop_duplicates_interface"]
    ],
)
def test_drop_duplicates_interface(
    interaction_file_interface,
    interaction_file_interface_aggbychain,
    kept_idxs_,
    kept_idxs_aggbychain_,
):
    interaction_file_interface = op.join(TEST_FILES_DIR, interaction_file_interface)
    interaction_file_interface_aggbychain = op.join(
        TEST_FILES_DIR, interaction_file_interface_aggbychain
    )
    ...
    (
        interaction_interface_unique,
        interaction_interface_aggbychain_unique,
    ) = structure_tools.drop_duplicates_interface(
        interaction_interface, interaction_interface_aggbychain
    )
